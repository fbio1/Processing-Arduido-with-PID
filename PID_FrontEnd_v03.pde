 ...
 **********************************************/
int windowWidth = 1100;      // set the size of the 
  ...
  try {
    ...
    if (s.length ==9) {
      ...
      if(justSent) {                     // * if this is the first read
        ...
        KnobSetPoint.setValue(Float.parseFloat(trim(s[1])));    //   the arduino,  take the
        InField.setText(trim(s[2]));    //   current values and put
        OutField.setText(trim(s[3]));   //   them into the input fields
        SliderKP.setValue(Float.parseFloat(trim(s[4])));     //
        SliderKI.setValue(Float.parseFloat(trim(s[5])));     //
        SliderKD.setValue(Float.parseFloat(trim(s[6])));    //
       // mode = trim(s[7]);              //
